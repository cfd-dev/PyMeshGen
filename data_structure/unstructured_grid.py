...
class Unstructured_Grid:
    ...
    def merge_elements(self):
        ...
        # 构建边到单元的映射
        for cell_idx, cell in enumerate(self.cell_container):
        ...
        for edge, cells in edge_map.items():
            ...
            cell1, cell2 = cells
            tri1 = self.cell_container[cell1]
            tri2 = self.cell_container[cell2]
        ...
        while merge_candidates:
            ...
            new_quad = Quadrilateral(
                ...
                node_coords[d],
                "interior",
                len(self.cell_container),
                [a, c, b, d],
            )

            # 替换原单元
            self.cell_container[cell1_idx] = new_quad
            self.cell_container[cell2_idx] = None  # 标记删除

            merged.update([cell1_idx, cell2_idx])
            num_merged += 1

        # 清理被删除的单元
        self.cell_container = [c for c in self.cell_container if c is not None]
