...
class BowyerWatsonTetGen:
    ...
    def _compute_max_edge_length_v2(tet):
        """计算四面体的最大边长（委托给 tet_utils）"""
        return compute_max_edge_length(tet)
    ...
    def _extract_surface_nodes(self):
        ...
        for tri in self.surface_triangles:
            for node3d in tri.nodes:
                nhash = node_hash(node3d.coords)
                if nhash not in node_hashes:
                    node_hashes.add(nhash)
    ...
    def _build_raycast_data(self):
        """构建射线投射数据（委托给 utils.ray_casting）"""
        n_tris = len(self.surface_triangles)
        if n_tris == 0:
            self._raycast_data = None
            return

        tri_verts = np.empty((n_tris, 3, 3), dtype=np.float64)
        for i, tri in enumerate(self.surface_triangles):
            for j in range(3):
                tri_verts[i, j] = tri.nodes[j].coords

        self._raycast_data = build_spatial_grid(tri_verts)

    def _classify_points_batch(self, points):
        """批量判断点是否在边界内部（委托给 utils.ray_casting）"""
        if self._raycast_data is None:
            self._build_raycast_data()
        if self._raycast_data is None:
            return np.ones(len(points), dtype=bool)
        return classify_points_inside(points, self._raycast_data)
    ...
    def _generate_interior_nodes(self):
        ...
        # 批量判断哪些点在边界内部
        inside_mask = self._classify_points_batch(all_points)
    ...
    def _create_super_tetrahedron(self):
        """创建超级四面体，包含所有节点（4.2节）"""
        super_coords = create_super_tetrahedron_coords(self.node_coords)
    ...
    def _in_circumsphere(self, node_coords, tet_coords):
        ...
        return in_circumsphere(node_coords, tet_coords)
    ...
    def _bowyer_watson(self, all_nodes):
        ...
        for step, node in enumerate(insert_nodes):
            ...
            for tet in cavity:
                for face_key in tet_faces(tet.node_ids):
    ...
    def _restore_surface_constraints(self):
        ...
        def rebuild_adjacency():
            """构建面->四面体 和 边->四面体 映射"""
            tets = [c for c in self.cell_container if isinstance(c, Tetrahedron)]
            face_to_tets = build_face_to_tets(tets)
            edge_to_tets = build_edge_to_tets(tets)
        ...
        def try_boundary_split(missing_face, edge_to_tets):
            ...
            for edge, tet_indices in edge_to_tets.items():
                ...
                if u >= -0.01 and v >= -0.01 and (u + v) <= 1.01:
                    ...
                    self.node_elem_by_hash[node_hash(new_node_coords)] = new_node
    ...
    def _laplacian_smooth(self, iterations=3):
        ...
        for ci, cell in enumerate(self.cell_container):
            if not isinstance(cell, Tetrahedron):
                continue
            has_boundary = any(
                self.node_elem_by_hash.get(node_hash(self.node_coords[nid])) is not None and
                self.node_elem_by_hash[node_hash(self.node_coords[nid])].bc_type == "wall"
                for nid in cell.node_ids
            )
            if has_boundary:
                for nid in cell.node_ids:
                    nh = node_hash(self.node_coords[nid])
                    node = self.node_elem_by_hash.get(nh)
    ...
    def _refine_oversized_tets(self, tets, all_nodes, super_ids, max_iterations=5):
        ...
        for iteration in range(max_iterations):
            ...
            for tet in oversized:
                ...
                if self._is_inside_boundary(center):
                    ...
                    if not too_close:
                        ...
                        self.node_elem_by_hash[node_hash(center)] = node
                        self.num_nodes += 1
            ...
            for node in new_nodes:
                ...
                # 找到腔体的边界面
                boundary_faces = find_boundary_faces(cavity)
    ...
    def generate(self, max_steps=None):
        ...
        self._build_raycast_data()
        info(f"表面三角形: {len(self.surface_triangles)}, 已构建射线投射索引")
